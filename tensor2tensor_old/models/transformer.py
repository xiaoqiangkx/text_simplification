...
def transformer_decoder2(decoder_input,
                        encoder_output,
                        decoder_self_attention_bias,
                        encoder_decoder_attention_bias,
                        rule_context,
        ...
        if encoder_output is not None:
          with tf.variable_scope("encdec_attention"):
            ...
            if layer == ctxly:
                contexts = tf.identity(y)
            x = common_layers.layer_postprocess(x, y, hparams)
        with tf.variable_scope("rule_attention"):
            y2 = common_attention.multihead_attention2(
                common_layers.layer_preprocess(
                    x, hparams), rule_context, rule_output, rule_bias,
                hparams.attention_key_channels or hparams.hidden_size,
                hparams.attention_value_channels or hparams.hidden_size,
                hparams.hidden_size, hparams.num_heads,
                hparams.attention_dropout,
            ...
            x2 = y
            x = tf.cond(
                tf.greater(global_step, tf.constant(2 * model_config.memory_prepare_step, dtype=tf.int64)),
                lambda: x2, lambda: x)
        with tf.variable_scope("ffn"):
          y = transformer_ffn_layer(
              common_layers.layer_preprocess(x, hparams), hparams,
