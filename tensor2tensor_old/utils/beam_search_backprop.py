...
def compute_topk_scores_and_seq(sequences, scores, scores_to_gather, flags,
                                ...
                                topk_decoder_logits=None,
                                topk_final_outputs=None):
  ...
  alive_decoder_logits = gather(topk_decoder_logits, "_topk_decoder_logits")
  alive_final_outputs = gather(topk_final_outputs, "_topk_final_outputs")

  return topk_seq, topk_gathered_scores, topk_flags, topk_gathered_states, alive_decoder_logits, alive_final_outputs
...
def beam_search(symbols_to_logits_fn,
  ...
  finished_scores = tf.zeros([batch_size, beam_size]) * -INF
  finished_flags = tf.zeros([batch_size, beam_size], tf.bool)

  initial_logits = tf.zeros((batch_size, vocab_size), dtype=tf.float32)
  alive_decoder_logits = tf.expand_dims(_expand_to_beam_size(initial_logits, beam_size) , axis=2) # (batch_size, beam_size, 1, vocab_size)
  finished_decoder_logits = tf.zeros(common_layers.shape_list(alive_decoder_logits), tf.float32) # (batch_size, beam_size, 1, vocab_size)

  initial_outputs = tf.zeros((batch_size, model_config.dimension), dtype=tf.float32)
  alive_final_outputs = tf.expand_dims(_expand_to_beam_size(initial_outputs, beam_size),
                                        axis=2)  # (batch_size, beam_size, 1, vocab_size)
  finished_final_outputs = tf.zeros(common_layers.shape_list(alive_decoder_logits),
                                     tf.float32)  # (batch_size, beam_size, 1, vocab_size)

  def grow_finished(finished_seq, finished_scores, finished_flags, curr_seq,
                    curr_scores, curr_finished,
                    topk_decoder_logits, finished_decoder_logits,
                    topk_final_outputs, finished_final_outputs):
    """Given sequences and scores, will gather the top k=beam size sequences.
    ...
    finished_decoder_logits = tf.concat(
        [finished_decoder_logits,
         tf.zeros([batch_size, beam_size, 1, vocab_size], tf.float32)], axis=2)

    finished_final_outputs = tf.concat(
        [finished_final_outputs,
         tf.zeros([batch_size, beam_size, 1, model_config.dimension], tf.float32)], axis=2)
    ...
    finished_decoder_logits = tf.concat([finished_decoder_logits, topk_decoder_logits], axis=1)
    finished_final_outputs = tf.concat([finished_final_outputs, topk_final_outputs], axis=1)

    return compute_topk_scores_and_seq(
        curr_finished_seq, curr_finished_scores, curr_finished_scores,
        curr_finished_flags, beam_size, batch_size, "grow_finished",
        topk_decoder_logits=finished_decoder_logits, topk_final_outputs=finished_final_outputs)

  def grow_alive(curr_seq, curr_scores, curr_log_probs, curr_finished, states, topk_decoder_logits, topk_final_outputs):
    """Given sequences and scores, will gather the top k=beam size sequences.
    ...
    return compute_topk_scores_and_seq(curr_seq, curr_scores, curr_log_probs,
                                       ...
                                       "grow_alive", states,
                                       topk_decoder_logits=topk_decoder_logits,
                                       topk_final_outputs=topk_final_outputs)

  def grow_topk(i, alive_seq, alive_log_probs, states, alive_decoder_logits, alive_final_outputs):
    ...
    if states:
      ...
      flat_logits, final_output = symbols_to_logits_fn(flat_ids)
    ...
    topk_decoder_logits = tf.gather_nd(alive_decoder_logits, topk_coordinates)
    cur_logits = tf.expand_dims(tf.gather_nd(curr_scores, topk_coordinates), axis=2)
    topk_decoder_logits = tf.concat([topk_decoder_logits, cur_logits], axis=2)

    topk_final_outputs = tf.gather_nd(alive_final_outputs, topk_coordinates)
    cur_outputs = tf.expand_dims(
        tf.gather_nd(tf.reshape(final_output, [batch_size, beam_size, model_config.dimension]), topk_coordinates),
        axis=2)
    topk_final_outputs = tf.concat([topk_final_outputs, cur_outputs], axis=2)

    return topk_seq, topk_log_probs, topk_scores, topk_finished, states, topk_decoder_logits, topk_final_outputs

  def inner_loop(i, alive_seq, alive_log_probs, finished_seq, finished_scores,
                 finished_flags, states,
                 alive_decoder_logits, finished_decoder_logits,
                 alive_final_outputs, finished_final_outputs):
    ...
    topk_seq, topk_log_probs, topk_scores, topk_finished, states, topk_decoder_logits, topk_final_outputs = grow_topk(
        i, alive_seq, alive_log_probs, states, alive_decoder_logits, alive_final_outputs)
    alive_seq, alive_log_probs, _, states, alive_decoder_logits, alive_final_outputs = grow_alive(
        topk_seq, topk_scores, topk_log_probs, topk_finished, states, topk_decoder_logits, topk_final_outputs)
    finished_seq, finished_scores, finished_flags, _, finished_decoder_logits, finished_final_outputs = grow_finished(
  ...
  finished_seq.set_shape((batch_size, beam_size, None))
  finished_decoder_logits.set_shape((batch_size, beam_size, None, vocab_size))
  finished_final_outputs.set_shape((batch_size, beam_size, None, model_config.dimension))
  ...
  finished_decoder_logits = tf.where(
      tf.reduce_any(finished_flags, 1), finished_decoder_logits, alive_decoder_logits)
  finished_final_outputs = tf.where(
      tf.reduce_any(finished_flags, 1), finished_final_outputs, alive_final_outputs)

  return finished_seq, finished_scores, finished_decoder_logits, finished_final_outputs