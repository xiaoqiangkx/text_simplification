...
def compute_topk_scores_and_seq(sequences, scores, scores_to_gather, flags,
  ...
  topk_indexes = tf.cast(tf.multinomial(scores, beam_size), tf.int32)
  ...
  return topk_seq, topk_gathered_scores, topk_flags, topk_gathered_states, alive_decoder_logits
...
def beam_search(symbols_to_logits_fn,
  ...
  alive_decoder_logits = tf.zeros(
      common_layers.shape_list(alive_seq), tf.float32) # (batch_size, beam_size, 1)
  finished_decoder_logits = tf.zeros(
      common_layers.shape_list(alive_seq), tf.float32)
  ...
  def grow_finished(finished_seq, finished_scores, finished_flags, curr_seq,
    ...
    finished_decoder_logits = tf.concat(
        [finished_decoder_logits,
         tf.zeros([batch_size, beam_size, 1], tf.float32)], axis=2)
  ...
  def grow_alive(curr_seq, curr_scores, curr_log_probs, curr_finished, states, topk_decoder_logits):
    """Given sequences and scores, will gather the top k=beam size sequences.
  ...
  def grow_topk(i, alive_seq, alive_log_probs, states, alive_decoder_logits):
    ...
    topk_ids = tf.cast(tf.multinomial(flat_curr_scores, beam_size * 2), tf.int32)
    topk_scores = tf.gather_nd(
        flat_curr_scores,
        tf.stack(
            [compute_batch_indices(batch_size, beam_size*2), topk_ids], axis=-1)
    )
    ...
    topk_decoder_logits = tf.concat(
        [topk_decoder_logits, tf.expand_dims(topk_log_probs, axis=2)],
        axis=2)

    return topk_seq, topk_log_probs, topk_scores, topk_finished, states, topk_decoder_logits
  ...
  def inner_loop(i, alive_seq, alive_log_probs, finished_seq, finished_scores,
    ...
    topk_seq, topk_log_probs, topk_scores, topk_finished, states, topk_decoder_logits = grow_topk(
        i, alive_seq, alive_log_probs, states, alive_decoder_logits)
    alive_seq, alive_log_probs, _, states, alive_decoder_logits = grow_alive(
        topk_seq, topk_scores, topk_log_probs, topk_finished, states,
        topk_decoder_logits)
    finished_seq, finished_scores, finished_flags, _, finished_decoder_logits = grow_finished(
  ...
  (_, alive_seq, alive_log_probs, finished_seq, finished_scores,
   finished_flags, _,
   alive_decoder_logits, finished_decoder_logits) = tf.while_loop(
       _is_finished,
       inner_loop, [
  ...
  finished_decoder_logits.set_shape((batch_size, beam_size, None))
  ...
  return finished_seq, finished_scores, finished_decoder_logits
